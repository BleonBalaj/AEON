...
export default function Globe(props:Props){
 ...
 try{renderer=new THREE.WebGLRenderer({antialias:true,alpha:true,powerPreference:'high-performance'});}catch{current.current.onBuffering?.(true);setError('3D rendering is unavailable on this device. The timeline and chapters remain available.');return;}
 let alive=true,contextUnavailable=false;current.current.onBuffering?.(true);const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
 ...
 renderer.domElement.addEventListener('pointerdown',pointerdown);renderer.domElement.addEventListener('pointerup',pointerup);renderer.domElement.addEventListener('keydown',key);const contextLost=(e:Event)=>{e.preventDefault();contextUnavailable=true;current.current.onBuffering?.(true);setError('The 3D view is recovering. Playback will resume when it is ready, or you can reload the globe.');};const contextRestored=()=>{if(!alive)return;contextUnavailable=false;setError('');last=0;};renderer.domElement.addEventListener('webglcontextlost',contextLost);renderer.domElement.addEventListener('webglcontextrestored',contextRestored);
 ...
 if(surfaceReady){
   ...
   if(!prewarmed&&requestedAge>.12){prewarmed=true;for(let gridAge=0;gridAge<=540;gridAge+=5)loadGrid(gridAge,false);}
 ...
 }else if(!lastLoading){setWaiting(true);current.current.onStatus('Updating reconstruction — holding the last loaded world.');current.current.onBuffering?.(true);lastLoading=true;}
