...
export default function Globe(props:Props){
 ...
 const scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(34,1,.1,100);camera.position.set(3.7,1.2,-1.65);const controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=!reduced;controls.dampingFactor=.07;controls.enablePan=false;let fitDistance=4;controls.minDistance=fitDistance;controls.maxDistance=fitDistance*1.8;controls.rotateSpeed=.55;controls.zoomSpeed=.7;controls.autoRotateSpeed=.24;
 ...
 let focusTarget:THREE.Vector3|null=null;current.current.onReady({zoom:f=>{camera.position.multiplyScalar(f);camera.position.clampLength(fitDistance,fitDistance*1.8);},reset:()=>{focusTarget=new THREE.Vector3(3.7,1.2,-1.65).normalize().multiplyScalar(fitDistance*1.1);},focus:(lat,lon)=>{focusTarget=xyz(lat,lon,fitDistance*1.1);}});
 ...
 const resize=new ResizeObserver(()=>{const w=el.clientWidth,h=el.clientHeight;if(!w||!h)return;renderer.setSize(w,h);camera.aspect=w/h;const halfFov=THREE.MathUtils.degToRad(camera.fov/2);const limitingAngle=Math.min(halfFov,Math.atan(Math.tan(halfFov)*camera.aspect));fitDistance=1.12/Math.sin(limitingAngle);controls.minDistance=fitDistance;controls.maxDistance=fitDistance*1.8;camera.position.clampLength(fitDistance,fitDistance*1.8);if(focusTarget)focusTarget.clampLength(fitDistance,fitDistance*1.8);camera.updateProjectionMatrix();});resize.observe(el);
