...
export default function HumanOdyssey(){
 ...
 return <main className="atlas odyssey">
  ...
  <section className="odyssey-console" aria-label="Human Odyssey playback"><div className="odyssey-playline"><div className="play-controls"><button className="step-button" aria-label="Previous human chapter" disabled={index===0} onClick={()=>go(index-1)}><ChevronLeft size={18}/></button><button className="play-button" aria-label={playing?'Pause Human Odyssey':'Play Human Odyssey demo'} onClick={play}>{playing?<Pause size={17}/>:<Play size={17}/>}</button><button className="step-button" aria-label="Next human chapter" disabled={index===chapters.length-1} onClick={()=>go(index+1)}><ChevronRight size={18}/></button></div><div className="odyssey-scrubber"><div><span>{atlasEvent?'Select Play to resume the guided journey':playing?'Following the journey':'Explore at your own pace'}</span><span>{atlasEvent?formatAge(atlasEvent.age):`${formatAge(chapter.start,true)} — ${formatAge(chapter.end,true)}`}</span></div><Slider aria-label="Human chapter progress" value={[progress]} min={0} max={100} step={.1} onValueChange={v=>{setAtlasEvent(null);setPlaying(false);setProgress(Array.isArray(v)?v[0]:v);}}/></div><button className="odyssey-restart" onClick={()=>{setAtlasEvent(null);setIndex(0);setProgress(0);setPlaying(true);}}><RotateCcw size={14}/> Restart demo</button></div><nav className="odyssey-chapters" aria-label="Human story chapters">{chapters.map((c,i)=><button key={c.id} aria-current={!atlasEvent&&i===index?'step':undefined} className={!atlasEvent&&i===index?'selected':''} onClick={()=>go(i)}><small>{String(i+1).padStart(2,'0')}</small><strong>{c.name}</strong><span>{c.region}</span><div className="odyssey-chapter-progress" style={{transform:`scaleX(${i===index?progress/100:i<index?1:0})`}}/></button>)}</nav></section>
