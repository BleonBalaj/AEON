...
## Historical tectonic mode — 2026-09-15

Added a separate CAO2024 tectonic globe when Plate tectonics is enabled between 0.3 and 1800 Ma. Matching coastline polygons and typed boundaries come from the explicitly selected GPlates CAO2024 model, rather than overlaying another model on PALEOMAP terrain. The cartographic surface has no invented elevation. Time is rounded to the nearest 1 Ma snapshot and exposed in the caption; this is not continuous plate motion. Present-day PB2002 and terrain mode remain available. Sources and limitations are in the atlas credits and layer description.

Geometry handles LineString, MultiLineString, Polygon and MultiPolygon, preserves rings, and tessellates great-circle edges across the date line. Fracture zones are not mislabeled as active transforms. Tests validate orientation, seam crossings, surface clearance, invalid geometry, and the downloaded 66/300/1000 Ma sample boundaries. Renderer batches by boundary color, bounds its cache to eight snapshots, cancels obsolete requests, times out after 20 seconds, retains/lists the previous displayed snapshot on failure, and offers retry or return to terrain. Context loss pauses drawing and requests a reload on restore.

TypeScript, the production build and existing data tests pass. Production-preview browser fixture test rendered a sourced 66 Ma sample, deliberately failed the next 300 Ma request, verified the visible old-snapshot notice, and returned successfully to 300 Ma terrain. Screenshot visually inspected. The external service was observed to time out in some earlier live requests; availability remains external, so this feature is not an offline reconstruction system. Continuous movement, velocity arrows, subduction-polarity markers and exhaustive mobile/device checks remain unfinished.

## Human Atlas metallurgy chapters — 2026-09-15

Added dedicated Bronze Age trade and Iron Age Hasanlu chapters with region-specific dates, source links and uncertainty, bringing the catalog to 50 chapters. Bronze Age coverage explains copper/tin bronze and eastern Mediterranean exchange using The Met's Cypriot ingot record. Iron Age coverage uses The Met's Hasanlu evidence to explain coexistence of iron with bronze and other materials. These are regional examples, not universal starting dates. Removed an irrelevant Iron Age keyword from the Olmec entry. Human Atlas introduction and search hint now include metalworking; Earth explorer still filters out Civilization entries.

TypeScript, data checks and production build passed. Production-preview browser test searched both names, selected each entry, opened the evidence sheet, and checked initial scroll position. Final-state screenshot inspected with finite animations completed. Ordinary headless captures sometimes froze at transition time zero; this is recorded as a capture limitation, not a proven app CSS defect. No panel-style changes were made. These two examples improve Bronze/Iron Age coverage but do not constitute global regional histories.
